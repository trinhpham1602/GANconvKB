...
parser = ArgumentParser(
    "CapsE", formatter_class=ArgumentDefaultsHelpFormatter, conflict_handler='resolve')
...
parser.add_argument("--num_filters", default=500, type=int,
                    help="Number of filters per filter size")
parser.add_argument("--dropout_keep_prob", default=1.0,
                    type=float, help="Dropout keep probability")
parser.add_argument("--l2_reg_lambda", default=0.001,
                    type=float, help="L2 regularization lambda")
...
parser.add_argument("--neg_ratio", default=1.0, type=float,
                    help="Number of negative triples generated by positive")
parser.add_argument("--use_pretrained", default=True,
                    type=bool, help="Using the pretrained embeddings")
...
parser.add_argument("--testIdx", default=1, type=int,
                    help="From 0 to 7. Index of one of 8 parts")
...
train, valid, test, words_indexes, indexes_words, \
    headTailSelector, entity2id, id2entity, relation2id, id2relation = build_data(
        path=args.data, name=args.name)
data_size = len(train)
...
if args.use_pretrained == True:
    ...
    lstEmbed = np.empty(
        [len(words_indexes), args.embedding_dim]).astype(np.float32)
    initEnt, initRel = init_norm_Vector(args.data + args.name + '/relation2vec' + str(args.embedding_dim) + '.init',
                                        args.data + args.name + '/entity2vec' +
                                        str(args.embedding_dim) + '.init',
                                        args.embedding_dim)
...
if args.decode == False:
    ...
    for _model_name in lstModelNames:
        out_dir = os.path.abspath(os.path.join(
            args.run_folder, "runs", _model_name))
        print("Evaluating {}\n".format(out_dir))
            ...
            for _index in range(args.num_splits):
                with open(_file + '.eval.' + str(_index) + '.txt') as f:
                    for _line in f:
                        if _line.strip() != '':
                            lstHT.append(
                                list(map(float, _line.strip().split())))
            lstHT = np.array(lstHT)
            print(_file, 'mr, mrr, hits@10 --> ',
                  np.sum(lstHT, axis=0) / (2 * len_test))
                ...
                for _model_index in lstModelIndexes:
                    ...
                    def test_prediction(x_batch, y_batch, head_or_tail='head'):
                        ...
                        for i in range(len(x_batch)):
                            new_x_batch = np.tile(
                                x_batch[i], (len(entity2id), 1))
                            new_y_batch = np.tile(
                                y_batch[i], (len(entity2id), 1))
                            ...
                            new_x_batch = np.insert(
                                new_x_batch, 0, x_batch[i], axis=0)
                            new_y_batch = np.insert(
                                new_y_batch, 0, y_batch[i], axis=0)
                            ...
                            results = []
                            listIndexes = range(0, len(new_x_batch), (int(
                            ...
                            results_with_id = rankdata(
                                results, method='ordinal')
                        ...
                        mr = mr/len(x_batch)
                        mrr = mrr/len(x_batch)
                        hits10 = hits10/len(x_batch)
                        return np.array([mr, mrr, hits10])

                    if args.testIdx < (args.num_splits - 1):
                        head_results = test_prediction(
                            x_test[batch_test * args.testIdx: batch_test *
                                   (args.testIdx + 1)],
                            y_test[batch_test * args.testIdx: batch_test *
                                   (args.testIdx + 1)],
                            head_or_tail='head')
